...
it('<ResultItem /> should render base case correctly', () => {
  const wrapper = shallow(<ResultItem trace={trace} durationPercent={50} linkTo='/trace/ae33f593bf2d0fa7' />);
...
it('<ResultItem /> should not render any ServiceTags when there are no services', () => {
  const wrapper = shallow(<ResultItem trace={{ ...trace, services: [] }} durationPercent={50} linkTo='/trace/ae33f593bf2d0fa7' />);
  const serviceTags = wrapper.find(`[data-test="${markers.SERVICE_TAGS}"]`).find(Tag);
